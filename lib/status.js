...
memoryLimit = memoryLimit || 100000000;
...
async function checkStatus(){
    ...
    if(memory.free<memoryLimit){
      sendMessage(`内存预警 memory total:${format(memory.total)}, free:${format(memory.free)}, 使用率:${memory.percent}%`)
    }
